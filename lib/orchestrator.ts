import { getActiveMission, saveDb, getDb } from './db';
import { Mission, Task, Phase } from '@/types';

/**
 * The Supr Orchestrator evaluate the current state of a mission 
...
export async function orchestrateMission(missionId: string, prompt: string): Promise<string> {
  ...
  if (gatePhase) {
    return `[SUPR] We have reached a critical gate: "${gatePhase.name}". All sub-tasks are complete, but I require your approval to advance the Glidepath. Please review the findings in the Mission Packet.`;
  }
